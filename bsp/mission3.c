...
#include "mission3.h"
/* 参数 */
#define TARGET_DISTANCE_CM1  0.0f
#define TARGET_DISTANCE_CM2  0.0f
...
static uint32_t stop_start_time = 0;
 float current_delta_yaw3 ;  // 当前转过的角度（实时显示用）

/* 急弯状态变量 */
static SharpTurnState3 sharp_state = SHARP_DELAY3;
static uint8_t white_state_count = 0;  // 转向完成次数计数
static uint8_t run_turn_enabled3 = 1;     // 是否启用转向逻辑（停机标志）
static uint16_t stabilize_timer = 0;     // 脱轨稳定缓冲计时（200Hz计数）
int32_t last_turn_finish_pulse3 = 0; 
uint8_t mission3_complete = 0; 
...
float get_current_delta_yaw3(void)
{
    return current_delta_yaw3;
}
    ...
    switch (sharp_state)
    {
        ...
        case SHARP_TURN3:
        {
            ...
            if (remain <= 5.0f)
            {
                ...
                // 进入脱轨前进阶段
                stop_start_time = millis();
                sharp_state = SHARP_RELEASE3;
            }
            else
            {
                // PD闭环控制：P=角度误差，D=角速度阻尼
                float p_out = yaw_err * 3.0f;
        ...
        case SHARP_RELEASE3:
        {
            ...
            if (millis() - stop_start_time >= 30)
            {
                ...
                last_turn_finish_pulse3 = (NEncoder.left_motor_total_cnt + NEncoder.right_motor_total_cnt) / 2; 							
                // 300ms后进入稳定缓冲期（防止巡线算法猛转）
                sharp_state = SHARP_DELAY3;
...
void run_turn_logic3_200hz(void)
{
    ...
    if (gray_state.state == 0x0000 || encoder_recorded)
    {
        ...
        if (sharp_state == SHARP_DELAY3 && !encoder_recorded)
        {
...
void stop3 (void)
{
		uint8_t turn_count = get_white_state_count3();

		if (turn_count >= 5 && mission3_complete == 0)
		{
				mission3_complete = 1;      // 进入第一阶段：主动刹车/倒车
				set_run_turn_enabled3(0);   // 禁用巡线转向逻辑
				stop_state_timer3 = 0;      // 清零计时器
...
// 倒车与停车状态机（不阻塞主循环）
		if (mission3_complete == 1)
		{
				...
				{
						mission3_complete = 2; // 倒车结束，进入彻底停止阶段
				}
		}
		else if (mission3_complete == 2)
		{		
				// 彻底停车锁死
				speed_expect[0] = 0;
				speed_expect[1] = 0;
    ...
    else if (mission3_complete == 0) 
    {
        // 1. 获取当前平均脉冲位置
        int32_t current_avg_pulse = (NEncoder.left_motor_total_cnt + NEncoder.right_motor_total_cnt) / 2;
        
        // 2. 计算距离上一次转弯结束跑了多少脉冲
        int32_t distance_pulse = current_avg_pulse - last_turn_finish_pulse3;
...
void mission3_show (void)
{
    ...
    if (millis() - last_show > 200)
    {
        ...
        SharpTurnState3 turn_state = get_sharp_turn_state3();
        switch (turn_state)
        {
            case SHARP_DELAY3:  state_str = "DELAY    "; break;
            case SHARP_WAIT3:   state_str = "WAIT     "; break;
            case SHARP_TURN3:   state_str = "TURN     "; break;
            default:           state_str = "NORMAL   "; break;
        }
        sprintf(buf, "State:%s", state_str);
        LCD_P6x8Str(0, 2, (unsigned char*)buf);

        float relative = get_current_delta_yaw3();  // 显示当前转过的角度
        float target = get_target_angle3();
        ...
        // 可选：显示转向计数
        sprintf(buf, "Turn:%d", get_white_state_count3());
